...
DB `kunde` mit dem DB-User `kunde`anzulegen. Mit dem 2. Skript wird das Schema
`kunde` mit dem DB-User `kunde` als _Owner_ angelegt. Abschließend werden die
Tabellen angelegt und mit Testdaten aus den CSV-Dateien aus dem Verzeichnis
`/init/kunde/csv` im Named Volume `pg_init` gefüllt.
...
```shell
    docker compose exec db bash
        ...
        psql --dbname=kunde --username=kunde --file=/init/kunde/sql/create-table.sql
        psql --dbname=kunde --username=postgres --file=/init/kunde/sql/copy-csv.sql
        exit
    docker compose down
```
